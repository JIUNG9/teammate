...
Calls ``teammate validate --json`` and ``teammate doctor --json`` as
...
from teammate.agent.base import FAIL, OK, WARN, RoutineConfig, RoutineResult

# Subprocess form: invoke teammate via ``python -m teammate.cli`` so we
# don't depend on the ``teammate`` script being on PATH inside whatever
# runner ends up calling us. Tests patch this list to avoid the real
# subprocess.
_TEAMMATE_CMD: tuple[str, ...] = (sys.executable, "-m", "teammate.cli")
...
def _run_subcommand(args: list[str], cwd: Path, timeout: int = 60) -> tuple[int, str, str]:
    """Run ``teammate <args...>`` and return ``(rc, stdout, stderr)``.
