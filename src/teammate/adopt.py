"""`teammate adopt` — mid-project file migration into a team-brain layout.

Most teams don't start with teammate. They have years of markdown scattered
...
class AdoptPlan:
    ...
    def to_markdown(self) -> str:
        ...
        lines: list[str] = ["# teammate adopt — migration plan", ""]
...
def _bundled_template_dir() -> Path:
    ...
    Resolution order:
      ...
      2. Installed wheel (pip install) — `<sys.prefix>/share/teammate/team-brain-skeleton`
         per `pyproject.toml`'s `[tool.hatch.build.targets.wheel.shared-data]` entry.
      3. User-site install — `~/.local/share/teammate/team-brain-skeleton`.
    ...
    candidates = [
        ...
        Path(sys.prefix) / "share" / "teammate" / "team-brain-skeleton",
        Path(site.getuserbase()) / "share" / "teammate" / "team-brain-skeleton",
...
def _stamp_template_marker(text: str) -> str:
    """Inject `teammate_template: true` into frontmatter, merging if present."""
    fm, body = _split_frontmatter(text)
    fm["teammate_template"] = True
