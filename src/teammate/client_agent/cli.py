"""CLI: `teammate war join/leave/status`.
...
`join <incident_id>`:
  ...
  - Adds the teammate MCP server to the Claude Code MCP config
...
HOOK_NAME = "teammate-war-mirror"
MCP_NAME = "teammate-war"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="teammate-war", description="teammate war-room client agent")
    ...
    join.add_argument("--war-api-url", default=os.environ.get("TEAMMATE_WAR_API_URL",
                                                              "https://chat.teammate.your-domain.net"))
...
def _cmd_join(incident_id: str, war_api_url: str) -> int:
    ...
    hook_cmd_pre = f"teammate-client-hook --phase pre --incident {incident_id} --url {war_api_url}"
    hook_cmd_post = f"teammate-client-hook --phase post --incident {incident_id} --url {war_api_url}"
