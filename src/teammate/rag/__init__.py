...
The headline pillar. Without this, ``teammate`` is just a compliance scanner.
With this, the new SRE on day 1 can ``teammate ask "what's our current K-ISMS-P
...
- **Ollama as the LLM and embeddings host.** Ubiquitous, no API key, runs
  ...
  Both are configurable via env vars or ``teammate.toml``.
- **Sqlite-backed vector store.** No external vector DB. Embeddings are
  stored as binary blobs in a single sqlite file under ``.teammate-cache/``.
...
- **gbrain compatibility.** If ``gbrain`` binary is on PATH, ``teammate
  init`` offers to register the vault as a gbrain source. The two systems
  coexist — gbrain handles cross-machine sync, teammate's mini-RAG handles
