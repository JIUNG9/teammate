...
If ``gbrain`` binary is on PATH, ``teammate init`` offers to register the
team's vault as a gbrain source. The two systems coexist:
  ...
  - **teammate's mini-RAG** handles the local query path even if gbrain
    isn't installed.
...
def detect() -> GBrainStatus:
    ...
    if not binary:
        return GBrainStatus(
            ...
            binary_path=None,
            version=None,
            notes="No `gbrain` binary on PATH. teammate's built-in mini-RAG will run instead.",
        )
    try:
    ...
    return GBrainStatus(
        ...
        binary_path=binary,
        version=out,
        notes=f"gbrain detected ({out}). teammate will register the vault as a source on init.",
    )
